 ...
 * each PRELOAD tier's JS + data blob (matched by name, leniently). Used ONLY to
 * derive coarse {@link AssetProgress} events; best-effort, so an unmatched bundle
 * name is simply not reported here (init itself surfaces a real mismatch).
 *
 * On-demand tiers are DELIBERATELY excluded: M4 items 6–7 mount them LAZILY (the
 * §5.4 static \usepackage scan and the missing-file retry, at compile time), NOT
 * during init — so reporting them in the init bracket would falsely claim they
 * loaded at boot. Per-tier progress for a lazy on-demand load would need a
 * compile-time asset-progress signal from the worker (there is none in M4); it is
 * a deferred follow-up. `stats.bundlesLoaded` still reflects the tiers actually
 * mounted, so a host can observe an on-demand tier came in.
 */
